...
VR_alset(singl *alph, Sint *nalph)
{
  ...
  if(alph1 != NULL) alph1 = Realloc(alph1, *nalph, double);
  else alph1 = Calloc(*nalph, double);
  for (i = 0; i < *nalph; ++i) alph1[i] = alph[i];
}
...
cov(int n, double *d, int pred)
{
  ...
  for (i = 0; i < n; ++i) {
    ...
    id = (int) (dd / mm);
    p = dd / mm - id;
    if (pred && id == 0) p = 1;  /* Preserve nugget effect on
				   prediction */
...
/* Perform Householder orthogonalization on LF and solve
  ...
  for (i = 0; i < *n; ++i) wz[i] = z[i] - val(x[i], y[i], bz, np);
  solv(yy, wz, *n, l, l);
  fsolv(w, wz, *n, l);
  Free(lf);
  Free(nu);
...
/*  FMAT evaluates the 'regression' matrix F (scaled to [-1,+1]x[-1,+1] to
  ...
  xs = Calloc(*n, double);
  ys = Calloc(*n, double);
  for (i = 0; i < *n; ++i) dscale(x[i], y[i], &xs[i], &ys[i]);
  k1 = 0;
  for (i = 0; i <= *np; ++i)
    for (j = 0; j <= *np - i; ++j)
      for (k = 0; k < *n; ++k)
...
/*  Finds the Householder decomposition of the NxM matrix F, F is reduced
  ...
  *ifail = 0;
  for (k = 0; k < m; ++k) {
    ...
    k3 = k1 + k;
    c2 = abs9(f[k3]);
    for (i = k + 1; i < n; ++i) c2 = max9(c2, abs9(f[i + k1]));
    if (c2 < 1e-6) {
      *ifail = k + 1;
      return;
    }
    ...
    c1 = sqrt(c1);
    b[k] = c1 * (c1 + abs9(nu[k3]));
    if (nu[k3] >= 0.0) nu[k3] += c1;
    else nu[k3] -= c1;
    i1 = (k + 1) * (k + 2) / 2;
    i2 = k + 1;
    for (i = k; i < m; ++i) {
      i3 = i * n;
      c1 = 0.0;
      for (j = k; j < n; ++j) c1 += nu[k1 + j] * f[i3 + j];
      c1 /= b[k];
      r[i1 - 1] = f[i3 + k] - c1 * nu[k3];
      for (j = k; j < n; ++j) f[i3 + j] -= c1 * nu[k1 + j];
      i1 += i2;
      ++i2;
    }
  }
...
/*  house_rhs is used in conjunction with householder. Once the Householder
  ...
  w = Calloc(n, double);
  for (i = 0; i < n; ++i) w[i] = z[i];
  for (k = 0; k < m; ++k) {
    sum = 0.0;
    k1 = k * n;
    for (i = k; i < n; ++i) sum += nu[i + k1] * w[i];
    sum /= b[k];
    for (i = k; i < n; ++i) w[i] -= sum * nu[i + k1];
  }
  bsolv(beta, w, m, r);
  Free(w);
}
 ...
 */
  ...
  *ifail = 1;
  ...
  for (icol = 1; icol <= n; ++icol) {
	...
	++i1;
	if (i == irow) continue;
	if (abs9(l[i1]) < eps && abs9(l[m]) < eps) ++m;
	else {
	  w -= l[i1] * l[m];
	  ++m;
	}
    ...
    if (abs9(w) < abs9(a[k] * eps)) l[k] = 0.0;
    else {
      if (w <= 0.0) return;
      l[k] = sqrt(w);
    }
...
/*   Evaluates prediction variance at (XP,YP) */
  ...
  for (it = 0; it < *npt; ++it) {
	...
	for (k = 0; k < *n; ++k) w1[i1 - 1] -= l1f[k1++] * w2[k];
      }
    fsolv(w2, w1, *npar, r);
    sum = 0.0;
    for (i = 0; i < *npar; ++i) sum += w2[i] * w2[i];
    z[it] = sum1 + sum;
  }
  Free(w1);
  Free(w2);
