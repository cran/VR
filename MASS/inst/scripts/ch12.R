...
exp(c(t1$fit - 2*t1$se, t1$fit + 2*t1$se))
t1 <-  predict(motor.wei, data.frame(temp=130),
               type = "uquantile", p = 0.1, se = T)
exp(c(t1$fit - 2*t1$se, t1$fit + 2*t1$se))

#if(version$major >= 5 || (version$major==4 && version$minor >= 5))
...
stan2 <- data.frame(start=c(0,183), stop=c(183,2*365),
     event=c(0,0), year=c(4,4), age=c(50,50)-48, surgery=c(1,1),
     transplant=c(0,1))
summary(survfit(stan, stan2, individual=T, conf.type="log-log"))
...
aids1.surv
plot(aids1.surv, mark.time=F, lty=1:4, col=2:5, xscale=365.25/12,
...
c.age <- factor(as.numeric(age2), labels=c("0-15", "16-30", "31-40",
   "41-50", "51-60", "61+"))
table(c.age)
# tmp <- diag(6)
...
survreg(Surv(survtime+0.9, status) ~ state + T.categ + age, data=Aidsp)
...
lines(0:82, exp(zz$fit-1.96*zz$se.fit)/365.25, lty=3, col=2)
rug(Aidsp$age+runif(length(Aidsp$age), -0.5, 0.5), 0.015)
