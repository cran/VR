...
{
  int     npat, index, i, j, k, ntr = *pntr, nte = *pnte, nind,
          ntie, *ind;
  double  dm, dist, tmp;
  ...
  for (npat = 0; npat < nte; npat++) {
	...
	dist += tmp * tmp;
      ...
      if (dist <= dm * (1 + EPS)) {
	if (dist < dm * (1 - EPS)) nind = 0; else nind++;
	dm = dist;
	ind[nind] = j;
      }
    }
    for (i = 0; i < *nc; i++) votes[i] = 0;
/* nind is the number of tied minima, minus one */
    if (nind == 0)
      index = class[ind[0]];
    else {
      for (i = 0; i <= nind; i++) votes[class[ind[i]]]++;
      j = votes[0];
/*
...
{
  int     i, index, j, k, k1, kinit = *kin, kn, l = *lin, mm,
          npat, ntie, ntr = *pntr, nte = *pnte, extras;
  int     pos[MAX_TIES];
  int     j1, j2, needed, t;
...
/* Use reservoir sampling to choose amongst the ties */
	...
	if (++t * UNIF < needed) {
      ...
      extras = 0;
      for (j = j1; j < kinit; j++) votes[class[pos[j]]]++;
    }
/* Use reservoir sampling to choose amongst the ties */
    ntie = 1;
    if (l > 0) mm = l - 1 + extras; else mm = 0;
    index = 0;
    for (i = 1; i <= *nc; i++)
      if (votes[i] > mm) {
	ntie = 1;
...
VR_lvq3(double *alpha, double *win, double *epsilon, Sint *pn, Sint *p,
	...
	Sint *niter, Sint *iters)
{
  int     index, iter, j, k, n = *pn, ncodes = *pncodes, nindex,
          npat, ntmp;
  double  alpha_t;
  double  dist, dm, ndm, tmp;
